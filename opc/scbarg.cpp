...
/*	if ( bHorz ) gripper.left -= 1, gripper.right += 6;
	else gripper.top -= 1, gripper.bottom += 6;

	// Draw gripper box
	CGrDC::Box3d(	pDC->GetSafeHdc(), gripper, 2, 
					GetSysColor( COLOR_BTNHIGHLIGHT ), 
					GetSysColor( COLOR_BTNSHADOW ) );
	
	// Prepare to fill
	InflateRect( gripper, -2, -2 );
	if ( bHorz ) gripper.bottom -= 3; else gripper.right -= 3;

	// Fill gripper box
	if ( bHorz ) CGrDC::VertGradientFill(	pDC->GetSafeHdc(), gripper, 
											GetSysColor( COLOR_ACTIVECAPTION ), 
											GetSysColor( COLOR_INACTIVECAPTION ) );
	else CGrDC::HorzGradientFill(	pDC->GetSafeHdc(), gripper, 
									GetSysColor( COLOR_ACTIVECAPTION ), 
									GetSysColor( COLOR_INACTIVECAPTION ) );
*/					
